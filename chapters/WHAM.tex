...
\label{Eq:biasmd}
\end{equation}
where $W_{i}(\eta(\textbf{x}))$ is the biasing potential adding on the reaction coordinate $\eta$, e.g. $W_{i}(\eta)=\frac{1}{2}k_{i}(\eta-\eta_{i})^2$ can be a Harmonic form. From these simulations a set of normalized biased probability distributions ${\rho_{i}^{(b)}(\eta)}$ can be obtained.
\begin{equation}
\rho_{i}^{(b)}(\eta) = \frac{e^{-\beta U_{i}^{(b)}(\eta)}}{Q_{i}^{(b)}}
\label{Eq:bias}
\end{equation}
where $Q_{i}^{(b)}=\int e^{-\beta U_{i}^{(b)}(\eta)} d\eta = e^{-\beta f_{i}^{(b)}}$ and $f_{i}^{(b)}$ is the free energy coming from the adding of the biasing potential $W_{i}(\eta(\textbf{x}))$ to the reference potential $U_{0}(\textbf{x})$. 
The corresponding unnormalized unbiased probability distribution $\rho_{i}^{(u)}(\eta)$ from $ith$ simulation is defined as, 
\begin{align}
...
The weights of simulation windows are required to be normalized,
\begin{equation}
\sum_{i=1}^{S}p_{i}(\eta)=1
\label{Eq:p1}
\end{equation}
and these weights are chosen so as to minimize the statistical error made on the total unbiased probability distribution $\rho_{0}(\eta)$, that is, for any given value of $\eta$,
\begin{equation}
\frac{\partial(\sigma^2[\rho_{0}(\eta)])}{\partial p_{i}(\eta)}=0
\label{Eq:partialp}
\end{equation} 
Then, under minimizing the statistical error, $\rho_{0}(\eta)$ takes the form,
\begin{align}
...
\label{Eq:unbias02}
\end{align} 
where due to $\rho_{i}^{(b)}(\eta)$  in the $ith$ biased simulation obeying the Boltzmann distribution, $\rho_{i}^{(b)}(\eta)$ can be treated as a $\delta$ function,
\begin{equation}
\rho_{i}^{(b)}(\eta) \equiv \frac{1}{N_{i}} \sum_{l=1}^{N_{i}} \delta {(\eta-\eta_{i,l})} 
\label{Eq:delta01}
\end{equation} 
where $\eta_{i,l}$, which locates in reaction coordinate $\eta$, is the $lth$ configuration in the $ith$ biased simulation .
...
\label{Eq:fi01}
\end{align} 
The set of parameters ${f_{i}^{(b)}}$ appear on the left and on the right of Eq.~\ref{Eq:fi01}, which can be solved iteratively. A first guess of values ${f_{i}^{(b)0}}$ is used on the right of Eq.~\ref{Eq:fi01} to compute a new set of values ${f_{i}^{(b)1}}$, which is in turn used as the new guess to compute ${f_{i}^{(b)1}}$ and so on until convergence of the process. Then, the unbiased free energy corresponding the histogram $\eta$ can be calculated by,
\begin{equation}
