...
\begin{equation}
Q = \frac{1}{{h}^{3N}N!} \iint \exp\left[-\beta H(\mathbf{x},\mathbf{p}_{x})\right] \diff\mathbf{x}\diff \mathbf{p}_\mathbf{x}.
\label{Eq:FEM:TP:PF}
\end{equation}
...
\begin{equation}
\Delta A = -\frac{1}{\beta} \iint \exp[-\beta \Delta H(\mathbf{x},\mathbf{p}_{x})] P_{0}(\mathbf{x},\mathbf{p}_{x}) \diff\mathbf{x}\diff\mathbf{p}_\mathbf{x},
