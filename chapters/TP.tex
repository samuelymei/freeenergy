...
\begin{align}
	\Delta F_{BA^\prime}=&-\beta^{-1}\ln {\int e^{-\beta (U_B(\mathbf{y})-U_{A^\prime}(\mathbf{y}))}\eta_{A^\prime}(\mathbf{y})\diff \mathbf{y}}\notag\\
...
\begin{equation}
	\eta_{A^\prime}(\mathbf{y})=\rho_{A}(\mathbf{x})/J(\mathbf{x})
...
In 2009, Hahn and Then extended this idea and proposed a bidirectional formulation as a generalized Bennett Acceptance Ratio method\cite{HahnPRE2009}, which reads
\begin{equation}
	\sum_{j=1}^{n_1}\frac{1}{1+\frac{n_1}{n_0}\exp{\left(\beta\widehat{\Delta F}_{01}-\beta W_j^1\right)}}=	\sum_{i=1}^{n_0}\frac{1}{1+\frac{n_1}{n_0}\exp{\left(-\beta\widehat{\Delta F}_{01}+\beta W_i^0\right)}}.
\end{equation}
