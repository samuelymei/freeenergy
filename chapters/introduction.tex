...
\end{align}
where $\mathbf{J}$ is the Jacobian matrix upon changing from Cartesian to generalized coordinates with its element defined as $\left[\mathbf{J}(\xi)\right]_{ij}=\partial x_i/\partial \xi_j$. $|\mathbf{J}|$ is its determinant. Its gradient over $\xi$ is
\begin{align}
